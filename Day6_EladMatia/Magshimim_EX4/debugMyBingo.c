...
typedef struct bingoPlayer //i added type def so he can right bingoPlayer and not struct everytime
{
	char name[STR_LEN]; 
	char board[BOARD_SIZE][BOARD_SIZE];
}bingoPlayer;

typedef struct bingoGame //removed all the initializers because in structs we only put the variable name
{
	int* numsDrawn;
	int numOfNumsDrawn;
	bingoPlayer * players; 
...
void initBingoPlayer(bingoPlayer* pPlayer)
{
	...
	myFgets(pPlayer->name, STR_LEN); //because its a pointer you use ->

	for (i = 0; i < BOARD_SIZE; i++)
	{
		for (j = 0; j < BOARD_SIZE; j++)
		{
			pPlayer->board[i][j] = 0;//because its a pointer you use ->
		}
	}
...
int updateGame(bingoGame* game, int num)
{
	...
	game->numOfNumsDrawn++;
	game->numsDrawn = (int*)realloc(game->numsDrawn, game->numOfNumsDrawn*sizeof(int)); // should be 1 because we are adding one spot for another number
...
*/
void printBoards(bingoGame game)
{	// because this function recieves struct, not a pointer we need to user . and not ->
	int i = 0;
	for (i = 0; i < game.numPlayers; i++)
	{
		printf("%s's board:\n", game.players[i].name);
